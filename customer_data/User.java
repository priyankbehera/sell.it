...
public class User {
    ...
    public String isCustomerOrSeller(User user) {
        if (user.isInvisible()) {
            return null; // Return null for invisible users
        }
    ...
    //Reads a conversation from a given file and adds each message to the messages list.
    ...
    //Reads messages from the conversation history file and prints the sender and message content.
    ...
    //Removes a blocked user from the conversation history by deleting the corresponding files.
    ...
    //Blocks a user with the given email, adding them to the list of blocked users and removing them from the conversation history.
    ...
    //Makes the current user invisible to the specified user, provided the specified user is not blocked.
